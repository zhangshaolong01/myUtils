package com.zhang.utils.scvzerng.entity;
...
public class ColorBlock {
	...
	public boolean isPointMatch(int leftTop, int rightTop, int leftBottom, int rightBottom, int center) {
		return this.leftTop.getRgb() == leftTop && this.rightTop.getRgb() == rightTop
				&& this.leftBottom.getRgb() == leftBottom && this.rightBottom.getRgb() == rightBottom
				&& this.center.getRgb() == center;

	}
	
	public boolean isPointMatch(int leftTop) {
	...
	public boolean allMatch(Coordinate coordinate, int[][] screen, double percent) {
		...
		for (int x = 0; x < this.getWidth(); x++) {
			if (!this.matchColor(this.rgbs[x][centerY], screen[x + coordinate.getX()][centerY + coordinate.getY()]))
				return false;
		}
		for (int y = 0; y < this.getHeight(); y++) {
			if (!this.matchColor(this.rgbs[centerX][y], screen[coordinate.getX() + centerX][y + coordinate.getY()]))
				return false;
		}
		// 检查y 中心线
		int temp = 0;
		for (int x = 0; x < this.getWidth(); x++) {
			for (int y = 0; y < this.getHeight(); y++) {
				if (this.matchColor(this.rgbs[x][y], screen[x + coordinate.getX()][y + coordinate.getY()]))
					temp++;
			}
		}
